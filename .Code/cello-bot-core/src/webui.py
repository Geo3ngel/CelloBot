...
class WSBroadcaster(ObserverClient):
    ...
    def handle_event(self, event_id, payload) -> None:
        if event_id == 'response_generated':
            global core_manager
            if core_manager.response_cancelled:
                return
            shutil.copy(core_manager.config.RESULT_TTSC, os.path.join(os.path.dirname(os.path.realpath(__file__)), 'static/generated/response.wav'))
...
def redirect_code():
    global core_manager
    core_manager.twitch_auth_code(request.args.get('code'))
...
def api_one_time_request():
    return base_api_no_response(
        core_manager.inject_one_time_request,
...
def api_get_name_translations():
    return base_api_with_response(
        core_manager.get_name_translations,
        no_toast=True
    )
...
def api_save_name_translations():
    return base_api_no_response(
        core_manager.save_name_translations,
...
def api_get_prompt_filenames():
    return base_api_with_response(
        core_manager.get_prompt_filenames,
        no_toast=True
    )
...
def api_get_current_prompt():
    return base_api_with_response(
        core_manager.get_current_prompt,
        no_toast=True
    )
...
def api_set_context_toggles():
    return base_api_no_response(
        core_manager.set_context_toggles, 
        params=[json.loads(request.data)],
...
def api_get_context_toggles():
    return base_api_with_response(
        core_manager.get_context_toggles,
        no_toast=True
    )
...
def api_get_config_from_file():
    return base_api_with_response(
        core_manager.get_config_from_file,
        params=[json.loads(request.data)['filename']],
...
def start_ui(app):
    ...
    core_manager = app
    page_params = {
        "twitch_relink_url": core_manager.twitch.OAUTH_AUTHORIZE_URL
    }
    event_client = WSBroadcaster(core_manager.broadcast_server)
